...
def train(initial,data,val,joint,seed=0,rank=64):
 torch.manual_seed(seed);base=copy.deepcopy(initial);adapter=Adapter(rank).to(next(base.parameters()).device) if joint else None
 opt=torch.optim.AdamW(list(base.parameters())+(list(adapter.parameters()) if joint else []),lr=1e-3,weight_decay=.01)
 gen=torch.Generator().manual_seed(seed);best=score(base,adapter,val);logs=[dict(epoch=0,val_mae=best)];chosen=0
